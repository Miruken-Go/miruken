...
func (suite *ProvidesTestSuite) TestProvides() {
	...
	suite.Run("OpenScoped", func () {
		...
		child := ctx.NewChild()
		baz, _, err = miruken.Resolve[*Baz](child, context.From)
		suite.Nil(err)
		suite.Equal(3, baz.Count())
		baz, _, err = miruken.Resolve[*Baz](child, context.From)
		suite.Nil(err)
		suite.Equal(3, baz.Count())
		bam, _, err = miruken.Resolve[*Bam](child, context.From)
		suite.Nil(err)
		suite.Equal(4, bam.Count())
		bam, _, err = miruken.Resolve[*Bam](child, context.From)
		suite.Nil(err)
		suite.Equal(4, bam.Count())
		baz, _, err = miruken.ResolveKey[*Baz](child, "Baz", context.From)
		suite.Nil(err)
