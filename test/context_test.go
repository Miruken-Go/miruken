...
func (suite *ContextTestSuite) TestContext() {
	...
	suite.Run("Traverse", func () {
		...
		suite.Run("Self", func() {
			...
			child := root.NewChild()
			root.Store(data)
			resolve, err := miruken.Resolve[*ContextObserver](miruken.BuildUp(child, miruken.SelfAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(root, miruken.SelfAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("Root", func() {
			...
			child := root.NewChild()
			child.Store(data)
			resolve, err := miruken.Resolve[*ContextObserver](miruken.BuildUp(child, miruken.RootAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			root.Store(data)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(root, miruken.RootAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("Children", func() {
			...
			child2.Store(data)
			resolve, err := miruken.Resolve[*ContextObserver](miruken.BuildUp(child2, miruken.ChildAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(grandChild, miruken.ChildAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(root, miruken.ChildAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("Siblings", func() {
			...
			grandChild := child3.NewChild()
			child3.Store(data)
			resolve, err := miruken.Resolve[*ContextObserver](miruken.BuildUp(root, miruken.SiblingAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(child3, miruken.SiblingAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(grandChild, miruken.SiblingAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(child2, miruken.SiblingAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("ChildrenOrSelf", func() {
			...
			grandChild := child3.NewChild()
			child3.Store(data)
			resolve, err := miruken.Resolve[*ContextObserver](miruken.BuildUp(child1, miruken.SelfOrChildAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(grandChild, miruken.SelfOrChildAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(child3, miruken.SelfOrChildAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(root, miruken.SelfOrChildAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("SiblingsOrSelf", func() {
			...
			grandChild := child3.NewChild()
			child3.Store(data)
			resolve, err := miruken.Resolve[*ContextObserver](miruken.BuildUp(root, miruken.SelfOrSiblingAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(grandChild, miruken.SelfOrSiblingAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(child3, miruken.SelfOrSiblingAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(child2, miruken.SelfOrSiblingAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("Ancestors", func() {
			...
			grandChild := child.NewChild()
			root.Store(data)
			resolve, err := miruken.Resolve[*ContextObserver](miruken.BuildUp(root, miruken.AncestorAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(grandChild, miruken.AncestorAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("AncestorsOrSelf", func() {
			...
			grandChild := child.NewChild()
			root.Store(data)
			resolve, err := miruken.Resolve[*ContextObserver](miruken.BuildUp(root, miruken.SelfOrAncestorAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(grandChild, miruken.SelfOrAncestorAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("Descendants", func() {
			...
			resolve, err := miruken.Resolve[*ContextObserver](miruken.BuildUp(grandChild, miruken.DescendantAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(child2, miruken.DescendantAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(child3, miruken.DescendantAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(root, miruken.DescendantAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("DescendantsOrSelf", func() {
			...
			grandChild := child3.NewChild()
			grandChild.Store(data)
			resolve, err := miruken.Resolve[*ContextObserver](miruken.BuildUp(child2, miruken.SelfOrDescendantAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(grandChild, miruken.SelfOrDescendantAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(child3, miruken.SelfOrDescendantAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(root, miruken.SelfOrDescendantAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("AncestorsSIBLINGSorSelf", func() {
			...
			grandChild := child3.NewChild()
			child2.Store(data)
			resolve, err := miruken.Resolve[*ContextObserver](miruken.BuildUp(grandChild, miruken.SelfSiblingOrAncestorAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, err = miruken.Resolve[*ContextObserver](miruken.BuildUp(child3, miruken.SelfSiblingOrAncestorAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
...
func (suite *ContextTestSuite) TestContextual() {
	...
	suite.Run("Resolve", func () {
		...
		suite.Run("ChildContextAssigned", func() {
			...
			suite.Nil(err)
			suite.NotNil(service)
			childService, err := miruken.Resolve[*ScopedService](
