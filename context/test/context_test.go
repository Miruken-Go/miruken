...
func (suite *ContextTestSuite) TestContext() {
	...
	suite.Run("GetSelf", func () {
		ctx := context.New()
		self, _, err := provides.Type[*context.Context](ctx)
		suite.Nil(err)
		suite.Same(ctx, self)
	})
	...
	suite.Run("Store", func () {
		...
		ctx  := context.New()
		ctx.Store(data)
		resolve, _, err := provides.Type[*ContextObserver](ctx)
		suite.Nil(err)
	...
	suite.Run("Traverse", func () {
		suite.Run("AncestorsByDefault", func() {
			...
			root.Store(data)
			resolve, _, err := provides.Type[*ContextObserver](grandChild)
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("Self", func() {
			...
			child := root.NewChild()
			root.Store(data)
			resolve, _, err := provides.Type[*ContextObserver](miruken.BuildUp(child, miruken.SelfAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(root, miruken.SelfAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("Root", func() {
			...
			child := root.NewChild()
			child.Store(data)
			resolve, _, err := provides.Type[*ContextObserver](miruken.BuildUp(child, miruken.RootAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			root.Store(data)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(root, miruken.RootAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("Children", func() {
			...
			child2.Store(data)
			resolve, _, err := provides.Type[*ContextObserver](miruken.BuildUp(child2, miruken.ChildAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(grandChild, miruken.ChildAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(root, miruken.ChildAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("Siblings", func() {
			...
			grandChild := child3.NewChild()
			child3.Store(data)
			resolve, _, err := provides.Type[*ContextObserver](miruken.BuildUp(root, miruken.SiblingAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(child3, miruken.SiblingAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(grandChild, miruken.SiblingAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(child2, miruken.SiblingAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("ChildrenOrSelf", func() {
			...
			grandChild := child3.NewChild()
			child3.Store(data)
			resolve, _, err := provides.Type[*ContextObserver](miruken.BuildUp(child1, miruken.SelfOrChildAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(grandChild, miruken.SelfOrChildAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(child3, miruken.SelfOrChildAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(root, miruken.SelfOrChildAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("SiblingsOrSelf", func() {
			...
			grandChild := child3.NewChild()
			child3.Store(data)
			resolve, _, err := provides.Type[*ContextObserver](miruken.BuildUp(root, miruken.SelfOrSiblingAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(grandChild, miruken.SelfOrSiblingAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(child3, miruken.SelfOrSiblingAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(child2, miruken.SelfOrSiblingAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("Ancestors", func() {
			...
			grandChild := child.NewChild()
			root.Store(data)
			resolve, _, err := provides.Type[*ContextObserver](miruken.BuildUp(root, miruken.AncestorAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(grandChild, miruken.AncestorAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("AncestorsOrSelf", func() {
			...
			grandChild := child.NewChild()
			root.Store(data)
			resolve, _, err := provides.Type[*ContextObserver](miruken.BuildUp(root, miruken.SelfOrAncestorAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(grandChild, miruken.SelfOrAncestorAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("Descendants", func() {
			...
			resolve, _, err := provides.Type[*ContextObserver](miruken.BuildUp(grandChild, miruken.DescendantAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(child2, miruken.DescendantAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(child3, miruken.DescendantAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(root, miruken.DescendantAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
		...
		suite.Run("DescendantsOrSelf", func() {
			...
			grandChild := child3.NewChild()
			grandChild.Store(data)
			resolve, _, err := provides.Type[*ContextObserver](miruken.BuildUp(child2, miruken.SelfOrDescendantAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(grandChild, miruken.SelfOrDescendantAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(child3, miruken.SelfOrDescendantAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		...
		suite.Run("AncestorsSIBLINGSorSelf", func() {
			...
			grandChild := child3.NewChild()
			child2.Store(data)
			resolve, _, err := provides.Type[*ContextObserver](miruken.BuildUp(grandChild, miruken.SelfSiblingOrAncestorAxis))
			suite.Nil(err)
			suite.Nil(resolve)
			resolve, _, err = provides.Type[*ContextObserver](miruken.BuildUp(child3, miruken.SelfSiblingOrAncestorAxis))
			suite.Nil(err)
			suite.Same(data, resolve)
		})
...
func (suite *ContextTestSuite) TestContextual() {
	...
	suite.Run("Resolve", func () {
		suite.Run("ContextAssigned", func() {
			...
			service2, _, err := provides.Type[*ScopedService](root)
			suite.Nil(err)
		...
		suite.Run("SameContextualWithoutQualifier", func() {
			...
			suite.Nil(err)
			suite.Same(root, service.Context())
			childService, _, err := provides.Type[*ScopedService](child)
			suite.Nil(err)
		...
		suite.Run("RootContextAssigned", func() {
			...
			child1 := root.NewChild()
			child2 := root.NewChild()
			service, _, err := provides.Type[*RootedService](child1)
			suite.Nil(err)
			suite.NotNil(service)
			suite.Same(root, service.Context())
			suite.False(service.disposed)
			service2, _, err := provides.Type[*RootedService](child2)
			suite.Nil(err)
			suite.Same(service, service2)
		})
	...
	suite.Run("Infer", func () {
		...
		suite.Run("Build", func () {
			suite.Run("Invariant", func() {
				root := suite.RootContext()
				foo, _, err := provides.Type[*Foo](root)
				suite.Nil(err)
				suite.Equal(1, foo.Count())
			})
			...
			suite.Run("Covariant", func() {
				...
				counter, _, err := provides.Type[Counter](root)
				suite.Nil(err)
		...
		suite.Run("RejectScopedDependencyInSingleton", func() {
			root := suite.RootContextWith(
				&ScopedService{},
				&LifestyleMismatch{})
			mismatch, _, err := provides.Type[*LifestyleMismatch](root)
			suite.Nil(err)
			suite.Nil(mismatch)
		})
	})
