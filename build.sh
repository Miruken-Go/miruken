docker run -v $(pwd):/go/src --workdir=/go/src golang:1.21.3-alpine3.18 go test ./...
if [[ $? -gt 0 ]]; then 
...
if [[ $(git tag -l "$TAG") ]];
    then
        ...
        gh workflow run update-team-api-dependencies.yml --repo https://github.com/Miruken-Go/demo.microservice -f mirukenVersion=$TAG
fi;
